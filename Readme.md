...
let parsed = scanf!(s, "Hello {}{}!", str, usize);
assert!(matches!(parsed, sscanf::Error::MatchFailed));
```
...
It also gives [worse compiler errors](https://docs.rs/sscanf/latest/sscanf/#a-note-on-compiler-errors) when using stable Rust.
...
and [`RegexRepresentation`](https://docs.rs/sscanf/latest/sscanf/trait.RegexRepresentation.html) for `scanf` to obtain the Regex of the Type:
```rust
struct TimeStamp {
