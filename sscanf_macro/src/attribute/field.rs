    ...
    fn from_attribute(attr: Attribute<attr::Field>, ty: &'_ syn::Type) -> Result<Self> {
        ...
        let ret = match kind {
            ...
            attr::Field::Map | attr::Field::FilterMap => {
                let filters = kind == attr::Field::FilterMap;
                ...
                let mapper = if let syn::Expr::Closure(closure) = mapper {
                    ...
                    let msg = format!("attribute `{kind}` requires a closure like: `{closure_format}`\n{closure_hint}");
                ...
                let param = if mapper.inputs.len() == 1 {
                    ...
                    let msg =
                        format!("attribute `{kind}` requires a closure with exactly one argument");
                ...
                let ty = if let syn::Pat::Type(ty) = param {
                    ...
                    let msg = format!("`{kind}` closure has to specify the type of the argument");
            ...
            attr::Field::From | attr::Field::TryFrom => {
                let hint = format!(
                    ...
                    if kind == attr::Field::From { "Into" } else { "TryInto" },
                    ty.to_token_stream()
                );
