...
use crate::{FromScanf, advanced::MatchTree};

/// A matcher that can be used to match input strings.
///
/// This is the type that the [`FromScanf::get_matcher`] method returns.
/// It is used to match parts of the input string in order to extract values. A call to the `sscanf` macro then
/// produce a [`MatchTree`] instance corresponding to the Matcher structure.
///
/// Take for instance the `Fraction` struct from the [`FromScanf`] documentation. It is set to
/// match an input that following the pattern `"{numerator}/{denominator}"`. This is turned into a `Matcher` instance
...
///
/// This form of compositing matchers creates a tree structure of matchers that can be used to match input strings and
/// still allow handing the extracted values back to the appropriate field parsers. So in this case, the `Fraction`
/// implementation's [`FromScanf::from_match_tree`] method is called with a
/// `MatchTree` containing child-trees for the numerator and denominator fields, which can then be passed to their respective
/// `from_match_tree` methods to extract the actual values.
#[derive(Debug, Clone)]
#[non_exhaustive]
pub enum Matcher {
    /// A raw regex matcher. Created using [`Matcher::from_regex`].
    ///
    /// Parsed using [`MatchTree::as_regex_matches`].
    Regex(RegexMatcher),
    /// Chain several parts together in sequence.
    ///
    /// Parsed using [`MatchTree::as_seq`].
    Seq(Vec<MatchPart>),
    /// Combine several matchers in a way that only one of them can match at a time.
    ///
    /// Parsed using [`MatchTree::as_alt`].
    Alt(Vec<Matcher>),
    /// An optional matcher.
    ///
    /// Parsed using [`MatchTree::as_opt`].
...
impl Matcher {
    ...
    ///
    /// Use [`MatchTree::as_regex_matches`] to extract the matches of any capture groups in the regex.
    ...
    ///
    /// Use [`MatchTree::as_opt`] to extract the inner match tree if it was matched.
