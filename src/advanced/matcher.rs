...
/// A call to the `sscanf` macro then produces a [`MatchTree`] corresponding to the matcher structure.
///
...
/// parsers. In this example, `Fraction`'s [`FromScanf::from_match_tree`] receives a `MatchTree` with child trees for
/// the numerator and denominator, which are then passed to their respective `from_match_tree` methods.
#[derive(Debug, Clone)]
#[non_exhaustive]
pub enum Matcher {
